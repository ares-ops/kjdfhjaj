...
function ClaimRewards() {
  ...
  const claimRewards = async () => {
    ...
    try {
      ...
      const tx = await program.methods
        .claimRewards(configKeys.configPdaBump)
        .accounts({
  ...
  const calculateAmountAfterTax = () => {
    ...
    const claimAmount = solToLamports(yourRewards);

    let fee = Math.floor(claimAmount * tokenTax + 99) / 100;

    if (fee > maximumFee) {
      fee = maximumFee;
    }

    return yourRewards - lamportsToSol(fee);
  };
    ...
    <div className="flex flex-col items-center justify-center h-screen px-4">
      <div className="bg-purple-light rounded-lg p-6 w-full max-w-md mx-auto">
        <div className="flex justify-between items-center">
          ...
          <Link to="/advanced-rewards-view" className="text-sm text-blue-300 cursor-pointer">
        ...
        <div className="mt-4">
          ...
          <div className="mt-6 flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0">
            {isLoading ? (
              ...
              <button className="bg-yellow-400 text-black px-6 py-2 rounded-lg w-full" onClick={claimRewards}>
                CLAIM
              </button>
            )}
          </div>
